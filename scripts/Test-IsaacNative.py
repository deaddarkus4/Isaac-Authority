...
RULES = dict(follow=1, behaviour=2, clear=4, taken=8, grid=16, fire=32, projectiles=64, tears=128, drops=256, counters=512, doors=1024, traps=2048, bombs=4096, hurt=8192, slots=16384, pets=32768, lead=65536, look=131072, join=262144)
...
attach(instances, 0x7FFFF & ~sum(RULES[name] for name in left_out), args.restart, args.carried, args.auto, args.installed, args.cheats)
folder = Path(os.environ["LOCALAPPDATA"]) / "IsaacAuthority"
games = []
for instance in instances:
    ...
    try:
        while time.monotonic() < deadline:
            ...
            for g, a, b in zip(games, now, previous):
                line.append(f"{g['title'][-1]}: FAULTS {a['gameFaults']}{(' (rules switched off: %x)' % a['faultRules']) if a['faultRules'] else ''}, ENEMY UPDATES THROWN {a['npcFaults']} (kinds left to themselves {a['ownBehaviourKinds']}), needle leaps set up {a['needleLeaps']}, newcomers let in {a['joinsAdmitted']} / refused {a['joinsRefused']} (lacking {a['joinLacking']}) / not judged {a['joinsUnjudged']}, datagrams refused {a['rejected']}, sent {(a['bytesSent'] - b['bytesSent']) / 10240:.1f} KB/s, received {(a['bytesReceived'] - b['bytesReceived']) / 10240:.1f} KB/s, frames lost in pieces {a['framesBroken']}, frames over 45 ms {a['longFrames'] - b['longFrames']} of {a['published'] - b['published']} (longest ever {a['frameMaxMs']} ms), bodies {a['applied'] - b['applied']} (max fix {a['correctionMax']:.1f}), enemies matched {a['npcMatched'] - b['npcMatched']}, "
