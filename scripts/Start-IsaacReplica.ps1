[CmdletBinding()]
param([string]$GameExecutable, [switch]$VerifyOnly)
...
$manifest = [ordered]@{createdUtc=[DateTime]::UtcNow.ToString('o'); role='replica-test'; workingDirectory=$working; saveDirectory=$save; gameExecutable=$GameExecutable; launch=$launch}
