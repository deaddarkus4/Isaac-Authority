...
$rules = [ordered]@{follow=1; behaviour=2; clear=4; taken=8; grid=16; fire=32; projectiles=64; tears=128; drops=256; counters=512; doors=1024; traps=2048; bombs=4096; hurt=8192; slots=16384; pets=32768; lead=65536; look=131072; join=262144; gate=524288; deal=1048576; hits=2097152; steady=4194304}
...
try {
    ...
    $mask = 0x7FFFFF; foreach ($name in $Without) { if ($name) { $mask = $mask -band (-bnot $rules[$name]) } }
    if ($mask -ne 0x7FFFFF) { [IO.File]::WriteAllText((Join-Path $settings 'native.cfg'), ('auto {0:x}' -f $mask), [Text.Encoding]::ASCII) }
