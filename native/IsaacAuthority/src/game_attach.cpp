...
int wmain(int argc, wchar_t** argv) {
    try {
        if (argc != 4) throw std::runtime_error("Usage: IsaacAuthorityAttach start|source|network|replica|world|world-stop|correct|stop PID DLL");
        ...
        const DWORD result = isaac_probe::InvokeProbeExport(pid, argv[3], entry, action == L"start" || action == L"network" || action == L"replica" || action == L"source" || action == L"world");
