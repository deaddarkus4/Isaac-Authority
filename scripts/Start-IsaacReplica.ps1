...
param([string]$GameExecutable, [switch]$VerifyOnly, [string[]]$GameArguments,
    ...
    [string]$PersistentGameData, [int]$WindowPosX = 960, [int]$WindowPosY = 0)
...
$manifest = [ordered]@{createdUtc=[DateTime]::UtcNow.ToString('o'); role='replica-test'; sharedSaveSha256=$sharedSaveSha256; gameArguments=@($GameArguments); workingDirectory=$working; saveDirectory=$save; gameExecutable=$GameExecutable; launch=$launch}
