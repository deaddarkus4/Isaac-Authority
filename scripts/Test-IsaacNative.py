...
COUNTERS = ("published", "received", "applied", "stale", "rejected", "otherRoom", "worldPublished", "worldReceived", "worldApplied", "npcMatched", "npcOnlyHost",
            ...
            "joinsAdmitted", "joinsRefused", "joinsUnjudged", "joinLacking")   # rejected: datagrams refused; gameFaults: exceptions on the game's thread - FAULTS; faultRules: the rules switched off after them
# The module's rules, each of which can be left out (--without): the same bits as in native_adapter.cpp.
RULES = dict(follow=1, behaviour=2, clear=4, taken=8, grid=16, fire=32, projectiles=64, tears=128, drops=256, counters=512, doors=1024, traps=2048, bombs=4096, hurt=8192, slots=16384, pets=32768, lead=65536, look=131072, join=262144, gate=524288)
...
attach(instances, 0xFFFFF & ~sum(RULES[name] for name in left_out), args.restart, args.carried, args.auto, args.installed, args.cheats)
folder = Path(os.environ["LOCALAPPDATA"]) / "IsaacAuthority"
games = []
for instance in instances:
