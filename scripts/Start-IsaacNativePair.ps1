...
for ($n = 0; $n -lt $Saves.Count; $n++) {
    ...
    # At once, before the next game starts: a game without the module would answer its Online menu with Steam. The local
    # user takes its localhost identity from the game's window, so the window has to exist first - it does by now.
    Start-Sleep -Seconds 3
    $attached = & (Join-Path $release 'IsaacAuthorityAttach.exe') localhost $instance.launch.pid $module | ConvertFrom-Json
    if ($attached.result -ne 0) { throw "The localhost service could not be set in ${title}: $($attached.result). Do not enter its Online menu." }
