...
COUNTERS = ("published", "received", "applied", "stale", "rejected", "otherRoom", "worldPublished", "worldReceived", "worldApplied", "npcMatched", "npcOnlyHost",
            ...
            "gameFaults", "faultRules", "floorFollows", "floorFollowFailures", "floorsElsewhere")   # rejected: datagrams refused; gameFaults: exceptions on the game's thread - FAULTS; faultRules: the rules switched off after them
    ...
    try:
        while time.monotonic() < deadline:
            ...
            for g, a, b in zip(games, now, previous):
                line.append(f"{g['title'][-1]}: FAULTS {a['gameFaults']}{(' (rules switched off: %x)' % a['faultRules']) if a['faultRules'] else ''}, datagrams refused {a['rejected']}, sent {(a['bytesSent'] - b['bytesSent']) / 10240:.1f} KB/s, received {(a['bytesReceived'] - b['bytesReceived']) / 10240:.1f} KB/s, frames lost in pieces {a['framesBroken']}, frames over 45 ms {a['longFrames'] - b['longFrames']} of {a['published'] - b['published']} (longest ever {a['frameMaxMs']} ms), bodies {a['applied'] - b['applied']} (max fix {a['correctionMax']:.1f}), enemies matched {a['npcMatched'] - b['npcMatched']}, "
