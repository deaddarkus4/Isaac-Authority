...
int wmain(int argc, wchar_t** argv) {
    try {
        if (argc != 4) throw std::runtime_error("Usage: IsaacAuthorityAttach start|source|network|replica|world|world-stop|input|input-stop|localhost|localhost-stop|native|native-stop|native-auto|correct|stop PID DLL");
        ...
        const char* entry = action == L"start" ? "IsaacAuthorityStart" :
            ...
            action == L"native" ? "IsaacAuthorityNativeStart" :
            action == L"native-stop" ? "IsaacAuthorityNativeStop" :
            action == L"native-auto" ? "IsaacAuthorityNativeAuto" :
        ...
        const DWORD result = isaac_probe::InvokeProbeExport(pid, argv[3], entry, action == L"start" || action == L"network" || action == L"replica" || action == L"source" || action == L"world" || action == L"input" || action == L"localhost" || action == L"native" || action == L"native-auto");
